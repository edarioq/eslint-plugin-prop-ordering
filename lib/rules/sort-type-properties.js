...
module.exports = {
  ...
  create: function (context) {
    ...
    function isCallback(propName) {
      ...
      // Check for "update" prefix followed by capital letter (updateUser but not updates)
      if (
        propName.length > 6 &&
        propName.startsWith("update") &&
        propName[6] === propName[6].toUpperCase()
      ) {
        return true;
      }
    ...
    function compareProps(a, b) {
      ...
      // sort by required vs optional
      const aIsOptional = isShorthand(a);
      const bIsOptional = isShorthand(b);
      
      if (shorthandFirst) {
        // Optional (shorthand) first when shorthandFirst is true
        ...
        // Required (non-optional) first when shorthandFirst is false
        if (!aIsOptional && bIsOptional) {
          return -1;
        }
        if (aIsOptional && !bIsOptional) {
